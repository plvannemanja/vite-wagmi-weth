...
export const SwapAmountProvider: React.FC<ContextProviderProps> = ({ children }) => {
	const { data: oraclePrice, isError: oracleError, isLoading: oracleLoading } = useReadContract({
	...
	const price = oraclePrice && Array.isArray(oraclePrice) ? oraclePrice[1] : null;
	...
	return (
		<SwapAmountContext.Provider value={{amount, setAmount: setTotalAmount, usdAmount, oracleError, oracleLoading}}>
			{children}
